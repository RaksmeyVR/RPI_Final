Public Class frm_subject
    ...
    '    Label_Credit.ForeColor = ThemeColor.PrimaryColor
    '    Label_Description.ForeColor = ThemeColor.SecondaryColor
...
#Region "Save data in form"
    Private Sub Insert()
        ...
        Sql.AddParam("@nameKH", txtSubject.Text)
        sql.AddParam("@nameEN", txtNameEN.Text)
        sql.AddParam("@credit", txtCredit.Text)
        ...
        sql.ExecQuery("INSERT INTO tbl_subject " &
                      "VALUES (@id,@nameKH,@nameEN,@credit,@semester,@year,@description);", True)
    ...
    Private Sub Update()
        ...
        sql.ExecQuery("Update tbl_subject SET nameKH= @nameKH,nameEN = @nameEN,credit = @credit,semester = @semester,year = @year ,description = @description WHERE id=@id")
    ...
    Private Sub Delete()
        If MsgBox("តើអ្នកប្រាកដទេថាចង់លុបទិន្នន័យនេះ !?", MsgBoxStyle.YesNo, "លុបទិនន្នន័យ?") = MsgBoxResult.Yes Then
            ...
            Sql.AddParam("@id", txtID.Text)
            DelString += "DELETE FROM tbl_subject WHERE id=@id ;"
...
#Region "Retreive Data in Form"
    ...
    Private Sub DGV_Data_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_Data.CellContentClick
        If e.RowIndex >= 0 Then
            ...
            Me.txtSubject.Text = row.Cells("nameKH").Value.ToString
            Me.txtNameEN.Text = row.Cells("nameEN").Value.ToString
            Me.txtCredit.Text = row.Cells("credit").Value.ToString
    ...
    Sub headdvg()
        ...
        DGV_Data.Columns(1).HeaderText = "មុខវិជ្ជា"
        DGV_Data.Columns(2).HeaderText = "ជាអក្សាឡាតាំង"
        DGV_Data.Columns(3).HeaderText = "ក្រេឌីត"
        DGV_Data.Columns(4).HeaderText = "ឆមាស"
        DGV_Data.Columns(5).HeaderText = "ឆ្នាំទី"
        DGV_Data.Columns(6).HeaderText = "ការពិពណ៌នា"
...
#End Region
    Sub autoid()
        ...
        txtID.Text = sql.getMaxID("tbl_subject", "id", "SJ-000", 4, 4)
    End Sub
